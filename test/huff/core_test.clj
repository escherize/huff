...
(deftest simple-html-test
  ...
  (is (= (h/html [:div {:style {:color "red" :background "black"}} "x" [:div {:style {:padding "10px"}} "z"] "y"])
         "<div style=\"color: red;background: black\">x<div style=\"padding: 10px\">z</div>y</div>"))
...
(deftest style-test
  (is (= (h/html [:div.c1#id1 {:style {:border "1px solid red"}} "x"])
         (h/html [:div.c1#id1 {:style "border: 1px solid red"} "x"]))))

(deftest prop-emission-test
  (is (= (#'h/emit-props {:id "x" :class ["x" "y"]}) " id=\"x\" class=\"x y\""))
  (is (= (#'h/emit-props {:id "x" :class []}) " id=\"x\"")))
